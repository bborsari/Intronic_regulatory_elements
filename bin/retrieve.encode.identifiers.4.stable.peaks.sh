...
# differs from the other because it keeps more than one 
# experiment for the same target and tissue
# (since we have individuals from EN-TEx)
...
awk -v format="$format" -v type="$type" -v assembly="$assembly" -v status="$status" -v assay="$assay" -v cell_line="$cell_line" 'BEGIN{FS=OFS="\t"} $2==format && $3==type && $5==assay && $7==cell_line && $38==assembly && $41==status {print $13, $4, $1, $25}' "$input" > "$outFile"
