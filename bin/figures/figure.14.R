...
my.function <- function(my.folder, my.table, my.xlab="") {
  ...
  # 1. read dataframes
  common.m <- read.table(paste0("intersection.with.adult/intronic/", my.folder, ".common.", my.table), h=F, sep="\t")
  colnames(common.m) <- c("region", "active_in_n_adult_tissues")
  common.m$group <- "common"
  
  differentiated_tissues.m <- read.table(paste0("intersection.with.adult/intronic/", my.folder, ".differentiated_tissues.", my.table), h=F, sep="\t")
  ...
  neural_progenitors.m <- read.table(paste0("intersection.with.adult/intronic/", my.folder, ".neural_progenitors.", my.table), h=F, sep="\t")
  colnames(neural_progenitors.m) <- c("region", "active_in_n_adult_tissues")
  neural_progenitors.m$group <- "neural_progenitors"
  
  stem_cells.m <- read.table(paste0("intersection.with.adult/intronic/", my.folder, ".stem_cells.", my.table), h=F, sep="\t")
...
# 4. plot
pdf("~/public_html/enhancers_neural_development/plots/fig.14.intronic.pdf", width = 13, height = 8)
