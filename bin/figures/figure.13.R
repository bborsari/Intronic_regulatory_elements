...
palette = c( "common" = "white",
             "stem_cells" ="gray70",
             "differentiated_tissues" = "forestgreen",
...
my.function1 <- function(folder) {
  ...
  for (i in 1:4) {
    ...
    my.list[[i]] <- read.table(paste0("/no_backup/rg/bborsari/projects/enhancers_neural_development/5-group.ccREs/hg19/embryo/", 
                                      folder, 
                                      "/",
                                      groups[i],
  ...
  my.df$group <- factor(my.df$group, levels = names(palette))
  
  p <- ggplot(my.df, aes(x=group, y=log10(gene_length+1), fill=group)) + 
    geom_boxplot() +
    stat_compare_means(comparisons = list(c("common", "stem_cells"),
                                          c("common", "differentiated_tissues"),
                                          c("common", "neural_progenitors")),
                       method = "wilcox.test",
                       size = 4.5) +
    theme_bw() + 
...
my.function2 <- function(folder) {
  ...
  m <- read.table("/no_backup/rg/bborsari/projects/enhancers_neural_development/5-group.ccREs/hg19/embryo/gencode.v19.number.introns.per.gene.tsv",
                  h=F, sep="\t")
...
my.function3 <- function(folder) {
  ...
  m <- read.table("/no_backup/rg/bborsari/projects/enhancers_neural_development/5-group.ccREs/hg19/embryo/gencode.v19.intron.sizes.tsv",
                  h=F, sep="\t")
...
groups <- c("common", "stem_cells", "neural_progenitors", "differentiated_tissues")
lop.analysis <- list()
...
pdf("~/public_html/enhancers_neural_development/plots/fig.13.analysis.pdf", width=9, height=4)
for ( i in 1:5) {
...
pdf("~/public_html/enhancers_neural_development/plots/fig.13.analysis.all.pdf", width=9, height=4)
for ( i in 1:5) {
