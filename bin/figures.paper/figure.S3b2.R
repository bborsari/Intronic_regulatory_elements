...
# 5. make plot
# pdf("~/public_html/enhancers_neural_development/figures.paper/fig.S3b2.pdf",
#     width = 4.5, height=3.5)
S3b2 <- ggplot(x, aes(x=group, y=log10(median+1), fill=group)) +
  geom_violin(alpha=.65, aes(color = group)) +
  geom_boxplot(width=0.3, alpha = .85) +
  stat_compare_means(label.x = 1, label.y = 7.5, size = 6) +
