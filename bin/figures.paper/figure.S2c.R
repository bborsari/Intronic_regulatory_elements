...
w <- read.table(url("https://public-docs.crg.es/rguigo/Data/bborsari/enhancers_neural_development/ccREs/hg19/gencode.v19.intron.sizes.tsv"), h=F, sep="\t")
...
# 7. make plot
# pdf("~/public_html/enhancers_neural_development/figures.paper/fig.S2c.pdf",
#     width = 4.5, height=3.5)
ggplot(x, aes(x=group, y=log10(median+1), fill=group)) +
  geom_violin(alpha=.65, aes(color = group)) +
  geom_boxplot(width=0.3, alpha = .85) +
  stat_compare_means(label.x = 1.5, label.y = 7.5) +
  annotate(geom="text", x=2.4, y=7, label=corr.v,
           color="black") +
