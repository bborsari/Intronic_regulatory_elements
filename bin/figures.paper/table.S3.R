...
source("/no_backup/rg/bborsari/projects/enhancers_neural_development/bin/figures.paper/figure.2a.R")
...
for (type in c("introns", "exons", "intergenic regions")) {
  ...
  for (group in (c("iPS", "fibro_myoblasts", "blood", "muscle", "brain"))) {
...
colnames(y)[1:2] <- c("genomic location", "cluster")
y$OR <- round(y$OR, 2)
